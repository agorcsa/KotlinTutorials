fun main() {
   ...
   // println(mary.weapon.damageInflicted)
    ...
    tim.inventory.add(Loot("+2 Ring of Protection", LootType.RING, 40.25))
    tim.inventory.add(Loot("Invisibility Potion", LootType.POTION, 35.95))

    tim.showInventory()

    // program looks in Player class to check if there is a toString fun to represent an object
    println(tim)
}
